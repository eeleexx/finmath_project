...
def test_hypothesis_1_divergence_spread():
    ...
    df["Spread"] = df["ImpliedVol_100"] - df["RV"]
...
def test_hypothesis_2_vol_smile():
    ...
    avg_iv_low = {
        "90 (Put OTM)": df_low["ImpliedVol_90"].mean(),
        "100 (ATM)": df_low["ImpliedVol_100"].mean(),
        "110 (Call OTM)": df_low["ImpliedVol_110"].mean(),
    }

    avg_iv_high = {
        "90 (Put OTM)": df_high["ImpliedVol_90"].mean(),
        "100 (ATM)": df_high["ImpliedVol_100"].mean(),
        "110 (Call OTM)": df_high["ImpliedVol_110"].mean(),
    }
